...
static int nec_parse_items(rmt_item32_t* item, int item_num, uint16_t* addr, uint16_t* data)
{
    int w_len = item_num;
    for( int x = 0; x < 1; ++x ) {
        ESP_LOGI(TAG, "ITEM%d uS %d %d & %d %d", x, NEC_ITEM_DURATION((item+x)->duration0), (item+x)->level0, NEC_ITEM_DURATION((item+x)->duration1), (item+x)->level1);
    }
    ...
    for( int x = 0; x < 1; ++x ) {
        // ESP_LOGI(TAG, "ITEM%d %d %d & %d %d", x, (item+x)->duration0, (item+x)->level0, (item+x)->duration1, (item+x)->level1);
        ESP_LOGI(TAG, "ITEM%d uS %d %d & %d %d", x, NEC_ITEM_DURATION((item+x)->duration0), (item+x)->level0, NEC_ITEM_DURATION((item+x)->duration1), (item+x)->level1);
    }
...
static int ltag_ir_rx_init()
{
    gpio_pad_select_gpio(RMT_RX_PIN);
...
static void nec_tx_init()
{
    ...
    rmt_tx.gpio_num = RMT_TX_PIN;
    rmt_tx.mem_block_num = 1;
    rmt_tx.clk_div = RMT_CLK_DIV;
    rmt_tx.tx_config.loop_en = false;
    rmt_tx.tx_config.carrier_duty_percent = 50;
    // rmt_tx.tx_config.carrier_freq_hz = 38000;
    rmt_tx.tx_config.carrier_freq_hz = 56000;
...
static void ltag_ir_recv_task()
{
    int channel = ltag_ir_rx_init();
    ...
    while(rb) {
        ...
        if(item) {
            uint16_t rmt_addr;
            uint16_t rmt_cmd;
            ESP_LOGI(TAG, "RMT RCV --- size: %d", rx_size);
